...
for i in range(epochs):
    batches_weak = dp.get_batches('cdr_train_weak', batch_size)
    batches_cdr = dp.get_batches('cdr_train', batch_size)
    batches_bc = dp.get_batches('bc_train', batch_size)

    l = min(len(batches_weak), len(batches_cdr), len(batches_bc))

    for j in range(l):
        loss_weak = train(batches_weak[j])
        ...
        if j % 100 == 0:
            ...
            if f1 > max_f1:
                save_path = saver.save(
                    sess, model_path + 'exp1_%d_%.1f.ckpt' % (hidden_size, args.dropout))
...
run_evaluation(train_batches_weak, 'cdr_train_weak')
