...
set_labels = {'cdr': ['Chemical', 'Disease'], 'bc': ['Chemical', 'Gene'],
              'weak': ['Disease', 'Chemical', 'Species', 'Gene',
                       'ProteinMutation', 'DNAMutation', 'SNP']}
...
dp.read_file(data_path + 'samples.txt', 'samples', 'weak')
...
def run_evaluation(batches, batch_size, extra_text=''):
    predictions = []
    batches_with_mask = []
    for batch in batches:
        token_batch, label_batch, shape_batch, char_batch, seq_len_batch, tok_len_batch, label_mask_batch = batch
        batch_seq_len, mask_batch, seq_len_batch = mask(batch, batch_size)
        ...
        for tokens, labels, prediction in zip(token_batch, label_batch, viterbi_repad):
            print([dp.inv_token_map()[t] for t in tokens])
            a = [dp.inv_label_map()[l] for l in labels]
            b = [dp.inv_label_map()[p] for p in prediction]
            print(a)
            print(b)
            print(a == b)
            print(['a', 'b'] == ['a', 'b'])
...
train_batches_weak = dp.get_batches('cdr_train_weak', batch_size, random=False)
...
print(len(samples_batches))
run_evaluation(samples_batches, 2)
