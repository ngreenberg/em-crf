...
def labels_mask_batch_to_array(batch):
    shape = (len(batch), max(map(len, batch)), len(batch[0][0]))
...
class DataProcessor(object):
    def __init__(self, set_labels, vocab=None, window_size=3):
        np.random.seed(42)
        ...
        self.label_map = {}
        self.label_map['O'] = len(self.label_map)
        for  l in set_labels.values():
            for label in l:
                for prefix in ['B-', 'I-', 'L-', 'O-', 'U-']:
                    if prefix + label not in self.label_map:
                        self.label_map[prefix + label] = len(self.label_map)

        self.labels_idx = {}
        for k, l in set_labels.items():
            self.labels_idx[k] = []
            for label, i in self.label_map.items():
                if label[2:] not in l:
                    self.labels_idx[k].append(i)
    ...
    def inv_label_map(self):
        return {i: l for l, i in self.label_map.items()}
    ...
    def make_example(self, key, lines, label_set, update):
        ...
        labels_mask = np.full((max_len_with_pad, len(self.label_map)), -np.inf)
        ...
        int_labels[self.pad_width:self.pad_width + len(labels)] = list(map(lambda s: self.label_map[s], labels))

        for i, label in enumerate(labels):
            if label == 'O':
                labels_mask[i + self.pad_width, self.labels_idx[label_set]] = 0
            else:
                labels_mask[i + self.pad_width, self.label_map[label]] = 0
        labels_mask[:self.pad_width, self.label_map['O']] = 0
        labels_mask[-self.pad_width:, self.label_map['O']] = 0

        padded_len = (len(sent_lens) + 1) * self.pad_width + sum(sent_lens)
        chars = chars[:sum(tok_lens)]

        example = (tokens, int_labels, shapes, chars, sent_lens, tok_lens, labels_mask)
    ...
    def get_batches(self, key, batch_size, random=True):
        ...
        labels_mask_batches = [labels_mask_batch_to_array([self.data[key][i][6] for i in b]) for b in batches]

        batches = [(token_batches[i], label_batches[i], shape_batches[i],
                    char_batches[i], sent_len_batches[i], tok_len_batches[i],
                    labels_mask_batches[i])
