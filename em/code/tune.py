...
parser.add_argument('--filename', default='exp.py', type=str)
...
slurm_cmd = 'srun --gres=gpu:1 --partition=%s --time=%s --mem=%s' % (partition, time, memory)
...
for log_str, setting_str in jobs_list.items():
    job_cmd = '--job-name="%s %s"' % (args.filename, setting_str)
    full_cmd = '%s %s %s %s' % (slurm_cmd, job_cmd, base_cmd, setting_str)
