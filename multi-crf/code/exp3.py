# pat 1: train on cid, test on bc and cdr
...
dp.read_file(cdr_path + 'ner_CDR_DevelopmentSet.PubTator.txt', 'cdr_dev', 'weak')
dp.read_file(cdr_path + 'ner_CDR_TestSet.PubTator.txt', 'cdr_test', 'weak')
dp.read_file(bc_path + 'ner_CDR_dev.txt', 'bc_dev', 'weak')
dp.read_file(bc_path + 'ner_CDR_test.txt', 'bc_test', 'weak')
        ...
        if j % 100 == 0:
            ...
            if f1 > max_f1:
                save_path = saver.save(
                    sess, model_path + 'exp3_%d_%.1f.ckpt' % (hidden_size, args.dropout))
...
run_evaluation(dev_batches_B, 'A', 'cdr_dev')
run_evaluation(dev_batches_C, 'A', 'bc_dev')

run_evaluation(test_batches_B, 'A', 'cdr_test')
run_evaluation(test_batches_C, 'A', 'bc_test')
