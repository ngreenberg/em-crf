...
parser.add_argument('-p', '--dropout', default=.75, type=float)
...
def run_evaluation(batches, batch_dataset, extra_text=''):
    ...
    for batch in batches:
        ...
        if viterbi:
            _, [probs_B], [probs_C] = sess.run(model.marginal_probabilities(), feed_dict=lstm_feed)
    ...
    predictions = (predictions_B, predictions_C)
    type_sets = (type_set_B, type_set_C)
    type_int_int_maps = (type_int_int_map_B, type_int_int_map_C)
    inv_label_maps = (inv_label_map_B, inv_label_map_C)

    merged_preds_list = []
    for (preds_B, preds_C, probs_B, probs_C) in zip(predictions_B, predictions_C, probabilities_B, probabilities_C):
        merged_preds_sublist = []
        for (pred_B, pred_C, prob_B, prob_C) in zip(preds_B, preds_C, probs_B, probs_C):
            ...
            for i, (b, c) in enumerate(zip(B, C)):
                if in_conflict and ((b == 'O' and c == 'O') or (beginning(b) and beginning(c))):
                    set_B = set(B[mention_start:i])
                    set_B.discard('O')
                    set_C = set(C[mention_start:i])
                    set_C.discard('O')

                    B_total = C_total = -np.inf
                    if len(set_B) != 0:
                        B_total = 0
                    if len(set_C) != 0:
                        C_total = 0
                    ...
                    ret[mention_start:i] = [B[mention_start:i], C[mention_start:i]][np.argmax([B_total, C_total])]

                    in_conflict = False

                if not in_conflict and (beginning(b) or beginning(c)):
                    mention_start = i

                in_conflict = in_conflict or b != c
