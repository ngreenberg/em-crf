...
parser.add_argument('-s', '--hiddensize', default=300, type=int)
parser.add_argument('-p', '--dropout', default=.5, type=float)
...
model = BiLSTM(
    ...
    num_classes_C=labels_full_size,
...
def run_evaluation(batches, batch_dataset, extra_text=''):
    ...
    for batch in batches:
        ...
        if viterbi:
            [probs_A], [probs_B], _ = sess.run(model.marginal_probabilities(), feed_dict=lstm_feed)
    ...
    inv_label_map_C = dp.inv_label_maps()['full']

    predictions = (predictions_A, predictions_B)

    merged_preds_list = []
    for (preds_A, preds_B, probs_A, probs_B) in zip(predictions_A, predictions_B, probabilities_A, probabilities_B):
        merged_preds_sublist = []
        for (pred_A, pred_B, prob_A, prob_B) in zip(preds_A, preds_B, probs_A, probs_B):
            ...
            for i, (a, b) in enumerate(zip(A, B)):
                if in_conflict and ((a == 'O' and b == 'O') or (beginning(a) and beginning(b))):
                    set_A = set(A[mention_start:i])
                    set_A.discard('O')
                    set_B = set(B[mention_start:i])
                    set_B.discard('O')

                    A_total = B_total = -np.inf
                    if len(set_A) != 0:
                        A_total = 0
                    if len(set_B) != 0:
                        B_total = 0
                    ...
                    ret[mention_start:i] = [A[mention_start:i], B[mention_start:i]][np.argmax([A_total, B_total])]

                    in_conflict = False

                if not in_conflict and (beginning(a) or beginning(b)):
                    mention_start = i

                in_conflict = in_conflict or a != b
    ...
    f1_micro, precision = evaluation.segment_eval(
        batches_with_mask, merged_preds_list, type_set_C, type_int_int_map_C,
        inv_label_map_C, dp.inv_token_map(),
        outside_idx=map(
            lambda t: type_set_C[t] if t in type_set_C else type_set_C['O'], outside_set),
