...
dp.read_file(cdr_path + 'ner_CID_Training_mine_PubTator.txt', 'cdr_train_weak', 'cid', update=True)
dp.read_file(cdr_path + 'ner_CDR_TrainingSet.PubTator.txt', 'cdr_train_gold', 'cid', update=True)
dp.read_file(cdr_path + 'ner_CDR_DevelopmentSet.PubTator.txt', 'cdr_dev', 'cid')
dp.read_file(cdr_path + 'ner_CDR_TestSet.PubTator.txt', 'cdr_test', 'cid')
dp.read_file(bc_path + 'ner_CDR_train.txt', 'bc_train', 'cid', update=True)
dp.read_file(bc_path + 'ner_CDR_dev.txt', 'bc_dev', 'cid')
dp.read_file(bc_path + 'ner_CDR_test.txt', 'bc_test', 'cid')
...
labels_cid_size = len(dp.label_maps['cid'])
...
for label, id in dp.label_maps['cid'].items():
...
def run_evaluation(batches, batch_dataset, extra_text=''):
    ...
    if batch_dataset == 'A':
        ...
        inv_label_map = dp.inv_label_maps()['cid']
    ...
    for j in range(l):
        ...
        if j % 250 == 0:
            f1_B, _ = run_evaluation(
                dev_batches_B, 'A', 'cdr_dev (iteration %d)' % i)
